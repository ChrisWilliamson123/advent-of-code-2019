class IntcodeComputer:
  ...
  def get_param_values(self, opcode, param1_mode, param2_mode):
    ...
    value1 = self.intcode[self.ip+1] if param1_mode else self.intcode[self.intcode[self.ip+1]]

    if opcode == 4:
      return [value1]
    else:
      value2 = self.intcode[self.ip+2] if param2_mode else self.intcode[self.intcode[self.ip+2]]
  ...
  def op_add(self, value1, value2, destination_address):
    self.intcode[destination_address] = value1 + value2

  def op_mul(self, value1, value2, destination_address):
    self.intcode[destination_address] = value1 * value2

  def op_input(self, destination_address):
    self.intcode[destination_address] = self.inputs.pop(0)
  ...
  def op_lt(self, value1, value2, destination_address):
    self.intcode[destination_address] = 1 if value1 < value2 else 0

  def op_eq(self, value1, value2, destination_address):
    self.intcode[destination_address] = 1 if value1 == value2 else 0

  def perform_next_operation(self):
    (opcode, [param1_mode, param2_mode]) = self.parse_operation_spec(self.intcode[self.ip])
    if opcode == 99:
      self.halted = True
      return
    ...
    if opcode == 1:
      self.op_add(values[0], values[1], self.intcode[self.ip+3])
    elif opcode == 2:
      self.op_mul(values[0], values[1], self.intcode[self.ip+3])
    elif opcode == 3:
      self.op_input(self.intcode[self.ip+1])
    elif opcode == 4:
      ...
      self.op_lt(values[0], values[1], self.intcode[self.ip+3])
    elif opcode == 8:
      self.op_eq(values[0], values[1], self.intcode[self.ip+3])
